...
not directly addressable; contents accessed with special instruction mfhi ("move from Hi")
and mflo ("move from Lo")
...
Program Structure
just plain text file with data declarations, program code (name of file should end in
suffix .s to be used with SPIM simulator)
...
value(s) usually gives initial value(s); for storage type .space, gives number of spaces to be
allocated
...
load address:
 la $t0, var1
copy RAM address of var1 (presumably a label defined in the program) into register
$t0
indirect addressing:
 lw $t2, ($t0)
...
recursive, return address should be copied from $ra onto stack to preserve it, since jal always
places return address in this register and hence will overwrite previous value
...
character.
If fewer than n-1 characters are in the current line, it reads up to and including the
newline and terminates the string with a null character.
